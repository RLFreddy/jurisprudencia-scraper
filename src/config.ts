...
// Single root for ALL storage (DB, files, errors). Point BASE_DIR to a
// mount in Docker (e.g. /app/data); on the host it defaults to ./output.
...
export const DB_PATH = process.env.DB_PATH ?? path.join(BASE_DIR, "scraper.db"); // SQLite state (WAL)
export const OUTPUT_DIR = process.env.OUTPUT_DIR ?? path.join(BASE_DIR, "files"); // output: files/page-N/
export const COOKIE_OVERRIDE = process.env.COOKIE_OVERRIDE ?? ""; // browser cookies for the WAF: __uzm*, uzmxj, etc.
export const ERROR_LOG_PATH = process.env.ERROR_LOG_PATH ?? path.join(BASE_DIR, "errors", "events.jsonl"); // event log (JSON lines, append)
export const ERROR_DIR = process.env.ERROR_DIR ?? path.join(BASE_DIR, "errors", "dumps"); // dumps of each failed response body
